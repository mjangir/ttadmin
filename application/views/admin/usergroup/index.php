    ...
    foreach ($data as $record) {
        ...
        ?>
                    <tr>
                        <td><?php echo $count;
        ?></td>
                        <td><?php echo $record->getGroupName();
        ?></td>
                        <td><?php echo $record->getRole()->getRole();
        ?></td>
        ...
        ?>" data-toggle="modal" data-modal-class="modal60p" data-target="#bjmodal"><i class="fa fa-eye text-info" data-toggle="tooltip" title="View"></i></a>&nbsp;
                            <a href="<?php echo $permissionUrl.'?id='.$id;
        ?>"><i class="fa fa-lock text-success" data-toggle="tooltip" title="Update Permissions"></i></a>&nbsp;
                            <?php if ($id !== 1 && $id !== 2) {
    ?>
                                <a href="<?php echo $updateUrl.'?id='.$id;
    ?>" data-toggle="modal" data-modal-class="modal40p" data-target="#bjmodal">
                                    <i class="fa fa-edit text-info" data-toggle="tooltip" title="Update"></i></a>&nbsp;
                                <a href="<?php echo $deleteUrl.'?id='.$id;
    ?>" class="ajax confirm" data-confirm-message="Are you sure, you want to delete this group?" data-method="post" data-refresh=".paginated_tbl">
