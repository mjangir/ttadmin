...
if (!$isAjaxRequest) {
    ...
    ?>
<form method="post" name="link_search_form" action="<?php echo $listingUrl;
    ?>" class="ajax listing-search" data-replace=".paginated_tbl" id="adsearchform">
    ...
    ?>
                ...
                <div class="col-xs-3 col-md-3 col-sm-4">
                    ...
                    <?php echo form_dropdown('parentId', array('' => 'Select Link') + $linkDropdown, $parentId, 'class="form-control link"');
    ?>
                </div>
                <div class="col-xs-3 col-md-3 col-sm-4">
                    <label class="control-label">Link Name</label>
                    <?php echo form_input(array('name' => 'name', 'class' => 'form-control', 'value' => $name));
    ?>
                </div>
                <div class="col-xs-3 col-md-3 col-sm-4">
                    <label class="control-label">Link Alias</label>
                    <?php echo form_input(array('name' => 'alias', 'class' => 'form-control', 'value' => $alias));
    ?>
                </div>
            </div>
        </div>
    ...
    foreach ($data as $record) {
        ...
        ?>
                    <tr>
                        <td><?php echo $count;
        ?></td>
                        <td><i class="<?php echo $record->getIcon();
        ?>"></i></td>
                        <td><?php echo $record->getName();
        ?></td>
                        <td><?php echo $record->getAlias();
        ?></td>
                        <td><?php echo $linkCategory;
        ?></td>
                        <td><span class="label label-<?php echo $labelClass;
        ?>"><?php echo $qualifiedStatus;
        ?></span></td>
                        <td>
                            <a href="<?php echo $viewUrl.'?id='.$id;
        ?>" data-toggle="modal" data-modal-class="modal60p" data-target="#bjmodal"><i class="fa fa-eye text-info" data-toggle="tooltip" title="View"></i></a>&nbsp;
                            <a href="<?php echo $updateUrl.'?id='.$id;
        ?>" data-toggle="modal" data-modal-class="modal60p" data-target="#bjmodal">
                                <i class="fa fa-edit text-info" data-toggle="tooltip" title="Update"></i></a>&nbsp;
                            <a href="<?php echo $deleteUrl.'?id='.$id;
        ?>" class="ajax confirm" data-confirm-message="Are you sure, you want to delete this link?" data-method="post" data-refresh=".paginated_tbl">
                                <i class="fa fa-trash-o text-danger" data-toggle="tooltip" title="Delete"></i></a>&nbsp;
                            <a href="<?php echo $statusUrl.'?id='.$id;
        ?>" class="ajax confirm" data-confirm-message="Are you sure, you want to make this link <?php echo $statusTitle;
        ?>?" data-method="post" data-refresh=".paginated_tbl">
                                <i class="fa fa-ban text-warning" data-toggle="tooltip" title="Make <?php echo $statusTitle;
        ?>"></i>
                        </td>
                    </tr>
                       <?php ++$count;
